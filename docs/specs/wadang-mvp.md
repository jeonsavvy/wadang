...
- 제목은 UTF-8 기준 1~80바이트, 안내문은 최대 280바이트입니다.
- 정원은 1~10,000이며 시작 시각은 종료 시각보다 빨라야 하고, 종료 시각은 생성 시점 이후여야 합니다.
- 시작 시각은 참여 가능 범위에 포함하고 종료 시각은 포함하지 않습니다.
- 참여할 때 설정된 verifier와 attester로 지갑의 현재 인증을 확인합니다.
...
프론트엔드는 확정된 영수증의 `CampaignCreated` 이벤트에서 캠페인 ID를 읽어 `/madang/{id}` 링크를 만듭니다. `campaignCount + 1`을 미리 추측하지 않습니다.
...
- 미인증 참여, 중복 참여와 비운영자 취소는 임의 주소의 RPC 시뮬레이션으로 확인합니다.
- 캠페인 2는 생성 후 운영자 지갑으로 취소합니다.
- 시간, 정원, 인증 취소와 verifier 오류는 로컬 테스트에서 경계값을 검증합니다.

## 배포 교체

배포된 컨트랙트는 삭제할 수 없습니다. 컨트랙트 변경이 필요하면 새로 배포하고 앱 설정의 주소와 문서를 함께 갱신합니다. Worker는 검증된 이전 버전으로 되돌릴 수 있습니다.
