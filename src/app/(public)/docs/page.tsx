...
const contractApi = [
  ...
  ["hasClaimed", "현재 인증 상태와 관계없이 과거 참여 여부를 반환합니다."],
  ["isEligible", "과거 참여, 캠페인 취소 여부와 현재 인증을 확인해 접근 가능 여부를 반환합니다."],
];
  ...
  return (
    <main className="page-shell container docs-page printable-doc">
      <header className="docs-head">
        <div><span className="product-kicker">기술 참조 · v0.4</span><h1>개발 문서</h1><p>컨트랙트 API, 참여 규칙, <code>isEligible</code> 연동과 테스트넷 실행 결과를 설명합니다.</p></div>
      ...
      <div className="docs-layout">
        <nav className="docs-nav print-hidden" aria-label="기술문서 목차">
          <div>
            ...
            <a href="#evidence"><span>06</span>검증 결과</a>
        ...
        <div className="docs-content">
          ...
          <section className="docs-summary-grid" aria-label="문서 요약">
            <article><ShieldCheck /><span>검증</span><strong>Dojang 상태 조회</strong><p>지정된 verifier와 attester로 참여 지갑의 현재 인증을 확인합니다.</p></article>
            <article><Database /><span>기록</span><strong>GIWA Sepolia</strong><p>기간, 정원, 참여 수, 취소 상태를 온체인에서 관리합니다.</p></article>
            <article><FileCheck2 /><span>연동</span><strong><code>isEligible</code></strong><p>외부 앱이 과거 참여와 현재 인증을 한 번에 조회합니다.</p></article>
          </section>
      ...
      <section className="docs-section" id="architecture">
        ...
        <p className="docs-copy">사용자 브라우저는 지갑과 GIWA Sepolia RPC에 직접 연결합니다. Cloudflare Worker는 화면과 정적 문서만 제공합니다. Dojang은 현재 인증 상태를, WadangCampaigns는 참여 규칙과 기록을 담당합니다. 외부 앱은 공개 조회 함수로 접근 가능 여부를 확인합니다. D1·KV·R2·Durable Objects는 사용하지 않습니다.</p>
      </section>
      ...
      <section className="docs-section" id="contract">
        ...
        <div className="invariant-grid">
          <div><strong>Dojang 인증</strong><p>참여할 때 verifier가 <code>true</code>를 반환해야 합니다. 조회 오류는 미인증이나 성공으로 처리하지 않습니다.</p></div>
      ...
      <section className="docs-section" id="integration">
        ...
        <p className="docs-copy">다른 컨트랙트는 WADANG의 상태를 복사하지 않고 <code>isEligible</code>을 읽어 접근을 결정할 수 있습니다. 로컬 테스트는 연동용 컨트랙트로 참여 전 거절, 참여 후 허용, 인증 취소 후 재거절을 확인합니다.</p>
...
if (!wadang.isEligible(campaignId, msg.sender)) {
    revert AccessDenied();
        ...
        <p className="docs-copy"><strong>v0.1 범위:</strong> 하나의 immutable verifier·attester 조합과 캠페인 참여 이력을 사용합니다. 파일럿 결과로 다중 인증, SDK·위젯과 Wallet 내 탐색 기능의 구현 순서를 정합니다.</p>
      </section>
      ...
      <section className="docs-section" id="security">
        ...
        <div className="boundary-grid">
          ...
          <div><h3>연동 서비스가 처리하는 것</h3><ul><li>인증 유형의 서비스별 해석</li><li>입장 이후 혜택 제공</li><li>추가 Sybil 방지 정책</li><li>멤버십 발급과 권한 설계</li><li>Wallet 내 탐색·알림 흐름</li></ul></div>
        </div>
      </section>

      <section className="docs-section docs-evidence" id="evidence">
        <div className="docs-section-title"><span>06</span><h2>검증 결과</h2></div>
        <div><Braces size={26} /><p><strong>로컬 테스트:</strong> 14개 컨트랙트 테스트가 한글 입력의 UTF-8 바이트 경계, 정원, 인증·미인증·중복, 시작·종료 시각, 취소 권한, 인증 취소·재발급, verifier 오류와 외부 접근 연동을 확인합니다. UI 테스트는 입력 제한, 화면 단계, 오류 안내와 생성 이벤트 ID 파싱을 다룹니다.</p></div>
        <div><ArrowUpRight size={26} /><p><strong>GIWA Sepolia:</strong> <code>{wadangRelease.contractAddress}</code>를 배포하고 Blockscout에서 소스를 검증했습니다. 캠페인 1은 생성과 참여 후 <code>isEligible == true</code>를 확인했고, 캠페인 2는 운영자 취소를 기록했습니다. 미인증 참여, 중복 참여와 비운영자 취소는 RPC 시뮬레이션에서 각각 <code>NotVerified</code>, <code>AlreadyClaimed</code>, <code>NotOrganizer</code>를 확인했습니다.</p></div>
      </section>

      <section className="docs-section mainnet-plan" id="next">
        <div className="docs-section-title"><span>07</span><h2>선발 후 검증</h2></div>
        <div><Cloud size={26} /><p><strong>90일 계획:</strong> GIWA Wallet에서 마당을 찾고 참여하는 흐름과 인증 만료·취소 안내를 검토합니다. 목표는 파일럿 1건, 인증 지갑 입장 100건, 반복 사용 운영자 3곳과 <code>isEligible</code> 외부 연동 1건입니다. 결과를 바탕으로 여러 Dojang 인증을 조합하는 자격 정책과 SDK·위젯의 범위를 결정합니다.</p></div>
      </section>
        </div>
      </div>
    </main>
