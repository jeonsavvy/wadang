...
export function formatContractError(error: unknown) {
  ...
  const matches: Array<[string, string]> = [
    ...
    ["AlreadyClaimed", "이미 이 캠페인에 참여한 지갑입니다. Explorer에서 기존 기록을 확인해 주세요."],
    ["CampaignNotStarted", "캠페인이 아직 시작되지 않았습니다. 시작 시각 이후 다시 시도해 주세요."],
    ["CampaignEnded", "참여 기간이 끝난 캠페인입니다."],
    ["CampaignFull", "참여 정원이 모두 찼습니다."],
    ["CampaignCanceled", "운영자가 닫은 캠페인입니다."],
    ["NotOrganizer", "이 캠페인을 만든 운영자 지갑만 닫을 수 있습니다."],
    ["CampaignNotFound", "캠페인을 찾지 못했습니다. 공유 링크의 ID를 확인해 주세요."],
    ["Failed to fetch", "GIWA RPC에 연결하지 못했습니다. 네트워크 상태를 확인하고 다시 시도해 주세요."],
    ["HTTP request failed", "GIWA RPC 응답이 지연되고 있습니다. 잠시 후 다시 시도해 주세요."],
  ];
  return matches.find(([needle]) => text.includes(needle))?.[1] ??
    "요청을 완료하지 못했습니다. 네트워크 상태와 지갑 로그를 확인한 뒤 다시 시도해 주세요.";
}
